...
public class MyPoint implements Serializable {
	...
	g.drawString(String.valueOf(index), startX + (int) p.x, (int) p.y + startY);
    }
    ...
    // ve hinh tron tuong trung dinh
    public void drawPoint(Graphics2D g, int index, Color colorPoint) {
	g.setColor(colorPoint);
	g.fill(el);
    }

    public void drawIndexCost(Graphics2D g, int index, String cost, Color colorIndex, Color colorCostResult) {
